...
dat_rs %>%
  ...
  select(N, manip, SD_manip, control, SD_control, Percent_control, 
         Author, Ecosystem_type, Soil_drainage) -> rs_growing_addition
...
dat_rs %>% 
  filter(depvar == "Rs_growingseason", manip != "NA", control != "NA", 
         Percent_control < 100) %>% 
  select(N, manip, SD_manip, control, SD_control, Percent_control, 
         Author, Ecosystem_type, Soil_drainage) -> rs_growing_removal
...
# Water addition
rsg_ma_add <- rma(yi, vi, mods = cbind(Percent_control, 
                  factor(rs_growing_addition$Ecosystem_type),
                  factor(rs_growing_addition$Soil_drainage)), 
                  data = rsg_es_addition)

# Water removal
rsg_ma_remove <- rma(yi, vi, mods = cbind(Percent_control,
                    factor(rs_growing_removal$Ecosystem_type),
                    factor(rs_growing_removal$Soil_drainage)), 
                    data = rsg_es_removal)
